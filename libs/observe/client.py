...
class AuthSession(requests.Session):
    def __init__(self, endpoint: str, ak: str, sk: str, workspace_id: str, app_id: str):
        ...
        self.observe_svc = ObserveService(
            endpoint=endpoint, region='cn-north-1')
...
class Client:
    def __init__(self, trace_endpoint: str, top_endpoint: str, ak: str, sk: str,
                 workspace_id: str, app_id: str, **kwargs):
        self.auth_session = AuthSession(
            top_endpoint, ak, sk, workspace_id, app_id)

        try:
            self.token = self.auth_session.get_token()
